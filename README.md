...
The first layer is the Conv2D layer with 200 filters and the filter size or the kernel size of 3X3. In this first step, the activation function used is the ‘ReLu’ and the pooling size is kept 2x2. This ReLu function stands for Rectified Linear Unit which will output the input directly if is positive, otherwise, it will output zero. The input size is also initialized as 150X150X3 for all the images to be trained and tested using this model

In the second layer,again the Conv2D layer is used but with 100 filters and the filter size or the kernel size of 3X3., the MaxPooling2D is used with the pool size of 2X2. The same 'Relu' activation function is used.
...
Finally, the CNN model along with the cascade classifier is trained for 20 epochs with two classes, one denoting the class of images with the face masks and the other without face masks and validation size of 20%. 
